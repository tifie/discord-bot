...
async def log_reaction(user_id: str, message_id: str, emoji: str):
    ...
    if not has_already_reacted(user_id, message_id, emoji):
...
async def save_user_data(user_data: dict):
    supabase.table("users").update(user_data).eq("id", user_data["id"]).execute()
...
async def main():
    add_user_if_not_exists("test_discord_id", "TestUser")
    points = get_total_points("test_discord_id")
